# Runs standalone via `Rscript tests/test_methodology_module.R` (repo root) and under
...
test_that("methodology_module contracts", {
  ...
  if (all(file.exists(c(helper_path, module_path)))) {
    ...
    source(formatting_path, local = TRUE)
    ...
    required_ui_text <- c(
      ...
      "Overview official burden snapshot",
      "Recent buyer empirical evidence",
      "Rental entry calculator pathway",
