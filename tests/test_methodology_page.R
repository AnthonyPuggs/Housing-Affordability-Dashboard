# Runs standalone via `Rscript tests/test_methodology_page.R` (repo root) and under
...
test_that("methodology_page contracts", {
  ...
  if (all(file.exists(c(app_path, module_path)))) {
    ...
    required_module_text <- c(
      ...
      "descriptive composite indicator",
      "Plain English Interpretation",
      "market-entry conditions are poor relative to the score history",
      "not a pass/fail affordability threshold",
      "does not mean housing is affordable for all households",
      "observed household burden, not market-entry conditions",
      "Rental-entry stress may be understated relative to advertised-rent or new-lease evidence",
      "public index-style inputs",
      "40/35/25",
      "relative to the score history",
      "historical-relative",
      "Score Diagnostics",
      "not an absolute affordability threshold",
      "ownership-channel overlap",
      "monthly servicing versus upfront deposit",
      "equal weights",
