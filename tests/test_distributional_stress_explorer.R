# Runs standalone via `Rscript tests/test_distributional_stress_explorer.R` (repo root) and under
...
test_that("distributional_stress_explorer contracts", {
  ...
  if (all(file.exists(c(module_path, chart_path, quality_path)))) {
    ...
    source(file.path(repo_root, "R", "ui_style_system.R"), local = TRUE)
