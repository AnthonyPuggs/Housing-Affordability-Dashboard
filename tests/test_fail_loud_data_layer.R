# Runs standalone via `Rscript tests/test_fail_loud_data_layer.R` (repo root) and under
...
test_that("fail_loud_data_layer contracts", {
  ...
  source(file.path(repo_root, "R", "project_paths.R"), local = TRUE)
  source(file.path(repo_root, "R", "pipeline_contracts.R"), local = TRUE)
  source(file.path(repo_root, "R", "data_loader.R"), local = TRUE)
