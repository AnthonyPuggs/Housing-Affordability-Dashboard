# Runs standalone via `Rscript tests/test_pipeline_outputs.R` (repo root) and under
...
test_that("pipeline_outputs contracts", {
  repo_root <- repo_root_path()
  source(file.path(repo_root, "R", "indicator_registry.R"), local = TRUE)
  source(file.path(repo_root, "R", "sih_benchmarks.R"), local = TRUE)

  read_required_csv <- function(filename) {
    path <- file.path(repo_root, "data", filename)
