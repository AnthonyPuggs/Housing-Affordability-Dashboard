# Runs standalone via `Rscript tests/test_provenance_report.R` (repo root) and under
...
test_that("provenance_report contracts", {
  ...
  if (all(file.exists(c(project_paths_path, registry_path, report_path,
    ...
    if (exists("methodology_provenance_report", mode = "function")) {
      ...
      required_report_text <- c(
        ...
        "front-page market-entry interpretation layer over the existing v2 composite",
