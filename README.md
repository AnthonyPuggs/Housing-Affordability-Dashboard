...
`R/methodology_module.R`, `R/affordability_module.R`, `R/rental_market_module.R`, `R/housing_supply_module.R` and `R/price_trends_module.R` are the first Shiny module pilots. They keep the registry-backed methodology/provenance page, Affordability page, Rental Market page, Housing Supply page and Price Trends page isolated while broader page modularisation remains incremental.
